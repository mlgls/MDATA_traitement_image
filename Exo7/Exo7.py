...
# Images test
label_test_data1 = [1, 3, 5, 7, 9, 14] # label des 6 images test du dataset 1
...
for img in path:
    n = cv2.imread(img, cv2.IMREAD_GRAYSCALE)
    img_data.append(n)
data_label = ([1] * 20) + ([3] * 20) + ([5] * 20) + ([7] * 20) + ([9] * 20) + ([14] * 20) # label des images du dataset
predicted = []
...
for index, img in enumerate(img_test):
    ...
    label_knn = res[:7] # on prend les k images les plus proches (k=10 pour 100% de précision)
    label = Counter([(item[1]) for item in label_knn]).most_common()[0][0] # on prend le label qui apparait le plus souvent
    print(label_knn)
    print("Catégorie de l'image :", label_test_data1[index])
    print("Prediction : ", label)
    predicted.append(label)

# Matrice de confusion
y_actu = pd.Series(predicted, name='Actual')
y_pred = pd.Series(label_test_data1, name='Predicted')
